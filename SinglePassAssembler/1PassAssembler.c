...
void main() {
    ...
    fsymt=fopen("symtab.txt","w+");
    f4=fopen("symtab1.txt","w+");
    f5=fopen("output.txt","w+");
    fscanf(finp,"%s%s%s",la,mne,opnd);
    ...
    while(strcmp(mne, "END")!=0) {
        if(strcmp(la, "-")==0) {
            ...
            fseek(fsymt,SEEK_SET,0);
            fscanf(fsymt,"%s%s",sym,symaddr);
            while(!feof(fsymt)) {
                if(strcmp(sym,la)==0) {
                    itoa(lc,lcs,10);
                    fprintf(f4,"%s\t%s\n",la,lcs);
